...
Explorer.afterDOMLoaded = concatenateResources(
  ...
  (function () {
    ...
    var _guardOverlay = null;
    function showBlockingOverlay() {
      if (_guardOverlay) return;
      _guardOverlay = (function(){
        var ov = document.createElement("div");
        Object.assign(ov.style, {
          position: "fixed", inset: "0",
          background: "rgba(0,0,0,.6)", zIndex: 9997,
    ...
    function hideBlockingOverlay() {
      if (_guardOverlay) {
        try { _guardOverlay.remove(); } catch {}
        _guardOverlay = null;
        document.body.style.overflow = "";
      }
    }

    var _accessModalOpen = false;
    function enforceAccess({ promptIfMissing = true } = {}) {
      try {
        var moduleName = getModuleFromPath();
        if (!moduleName) { hideBlockingOverlay(); return true; }
        if (hasAccess(moduleName)) { hideBlockingOverlay(); return true; }

        showBlockingOverlay();
        if (!promptIfMissing || _accessModalOpen) return false;

        _accessModalOpen = true;
        showPasswordModal(moduleName).then(function(ok){
          _accessModalOpen = false;
          if (ok) {
        ...
        }).catch(function(){ _accessModalOpen = false; });
    ...
    function startStableAccessMonitor() {
      ...
      window.addEventListener("pageshow", function(e){
        enforceAccess({ promptIfMissing: true });
      });

      // історія/хеш
      window.addEventListener("popstate", function(){ enforceAccess({ promptIfMissing: true }); });
      window.addEventListener("hashchange", function(){ enforceAccess({ promptIfMissing: true }); });
      ...
      // фокус / видимість
      window.addEventListener("focus", function(){ enforceAccess({ promptIfMissing: true }); });
      document.addEventListener("visibilitychange", function(){
        if (document.visibilityState === "visible") enforceAccess({ promptIfMissing: true });
      });

      // DOM-мутації (на випадок внутрішніх оновлень)
      try {
    ...
    startStableAccessMonitor();

    /* ===== /Stable Access Monitor ===== */
